...
$\frac{86400}{30}36 \cdot \textrm{NMACK}$}. Since this represents the minimum distance
that keys will have from the nearest KROOT, we can derive a loose lower bound on
how long it can take under these circumstances to verify a key against a
floating KROOT. Applying the same benchmark script as used in the previous
section, and applying the same performance conversion devised in
~\ref{eq:mips_conv}, we obtain the numbers in table ~\ref{table:float_kroot}.
...
\bottomrule
  \caption{Lower bounds for authentication of a key against a floating KROOT}
\label{table:float_kroot}
\end{longtable}

This is a definite improvement over the time it takes to authenticate a key at
the end of the chain with the root key at its beginning, but given this is a
lower range (even if loose) it still shows that authenticating a key coming from
a MACK section against a key authenticated by ground segment and sent in a KROOT
section is an expensive operation that might undermine the overall performance
of a receiver.
...
%   eliminating the need for floating KROOTs
