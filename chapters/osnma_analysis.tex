...
components. Starting from how OSNMA groups data for individual authentication,
we distinguish three main categories: ephemeris data, almanac data and
Galileo-GPS conversion parameters. In a 30s cycle, the navigation message
includes all the ephemeris for the transmitting satellite, the conversion
parameters and almanac data for one and a half satellites out of 36. In the same
time frame, depending on the configuration, 1 to 15 MACs can be transmitted.
This means that in some cases we could receive data in a subframe and its
authentication in a subsequent subframe, or the other way around. In any case
this means that the receiver needs to have a local cache that captures the
following:
\begin{itemize}
...
codes relative to the GPS navigation message, for all the possible 35 satellites
Galileo can authenticate.
...
Another important tradeoff that must be considered when designing a receiver is
related to the fact that Galileo can authenticate also a whole full subframe;
here the two opposing forces are memory and complexity. On one side, one could
allow for a complete copy of the subframe to be stored independently from the
other fields. The second approach is to collect the bits that compose the
subframe and that are not included in the other stored fields, and have the CPU
recompute the subframe structure prior to authentication. This last approach has
the advantage of saving memory, but requires some computation on the processor
side before the subframe could be authenticated. The first approach instead
removes the need of CPU cycles at the expense of more memory used and some
duplicated data. On the other hand, this approach also allows the receiver to
store ephemeris and almanac data independently from the data received in the
...
of the data presented above needs to be computed by the receiver, which needs
make sure enough CPU cylces are scheduled at the right time, without running
into the risk of blocking other operations. From this point 


% - MAC
%   - CTR
%   - GST_SF
%   - PRN satellite giving authentication information
% - NMA status header
% - key
% 
% Consider only 30sec cycles

% TODO IODnav -> how to verify if data needs authentication

% - MACs can be received at different times than the navigation message: how to
%   store them so that space is not wasted (i.e. should we store a MAC for a
%   satellite that might not be in view)?
% - a MAC can authenticate more than one field: how to efficiently store the MAC
%   and have the right fields be marked as authenticated?
