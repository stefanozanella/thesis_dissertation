...
\begin{itemize}
  ...
  \item $t_j$: the KROOT time (in seconds) calculated as combination of KROOT WN
    and KROOT DOW as described above
  \item $n_M$: the number of MACK sections sent in a subframe. This directly
    relates to the number of keys applied in a subframe as one MACK section
    contains always one key, and it's sent in the \textrm{NMACK} field in the
    HKROOT section
  \item $p$: the position of key $K_m$ in the subframe in which it's transmitted
    (i.e. how many keys are sent in the subframe before $K_m$)
\end{itemize}
Once these parameters are known, then it's possible to calculate the distance
between the keys as follows:
\begin{equation}
  \label{eq:d}
  d = \frac{t_m - t_j}{30} n_M + p
\end{equation}

Having achieved this result we can now analyze how to calculate $t_m - t_j$ and
$p$, since OSNMA doesn't send those times explicitly.
...
\begin{equation}
  \begin{aligned}
    ...
    \textrm{WN}_j \cdot 604800 + \textrm{DOW}_j \cdot 86400)}{30} n_M + p \\
    &= \frac{\textrm{WN}_m \cdot 604800 + \textrm{TOW}_m - \textrm{WN}_j \cdot
    604800 - \textrm{DOW}_j \cdot 86400}{30} n_M + p \\
    &= \frac{(\textrm{WN}_m - \textrm{WN}_j) \cdot 604800 + (\textrm{TOW}_m -
    \textrm{DOW}_j \cdot 86400)}{30} n_M + p
  \end{aligned}
\end{equation}

TODO Calculation of p
